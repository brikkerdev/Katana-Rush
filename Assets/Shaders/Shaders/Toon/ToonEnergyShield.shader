Shader "Lpk/LightModel/ToonEnergyShield"
{
    Properties
    {
        ...
        [Space]
        _ShieldColor        ("Shield Color", Color)               = (0,1,1,1)
        _ShieldOpacity      ("Shield Opacity", Range(0,1))        = 0
        _ShieldSpeed        ("Shield Speed", Range(0.1, 10))      = 1
        _ShieldSize         ("Shield Size", Range(0.01, 1))       = 0.1
        _ShieldOffset       ("Shield Offset", Range(-1, 1))       = 0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            UNITY_INSTANCING_BUFFER_START(Props)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float, _ShieldOpacity)
                UNITY_DEFINE_INSTANCED_PROP(float, _ShieldSpeed)
                UNITY_DEFINE_INSTANCED_PROP(float, _ShieldSize)
                UNITY_DEFINE_INSTANCED_PROP(float, _ShieldOffset)
            ...
            struct v2f
            {
                ...
                float3 objectPos    : TEXCOORD4; // Object space position for shield effect
                UNITY_FOG_COORDS(5)
                SHADOW_COORDS(6)
                UNITY_VERTEX_INPUT_INSTANCE_ID
            };
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float4 shieldColor = UNITY_ACCESS_INSTANCED_PROP(Props, _ShieldColor);
                float shieldOpacity = UNITY_ACCESS_INSTANCED_PROP(Props, _ShieldOpacity);
                float shieldSpeed = UNITY_ACCESS_INSTANCED_PROP(Props, _ShieldSpeed);
                float shieldSize = UNITY_ACCESS_INSTANCED_PROP(Props, _ShieldSize);
                float shieldOffset = UNITY_ACCESS_INSTANCED_PROP(Props, _ShieldOffset);
                ...
                float shieldEffect = smoothstep(shieldOffset - shieldSize * 0.5, shieldOffset + shieldSize * 0.5, fmod(i.objectPos.y + _Time.y * shieldSpeed, 1.0));
                finalColor = lerp(finalColor, shieldColor.rgb, shieldEffect * shieldOpacity);
                finalAlpha = max(finalAlpha, shieldEffect * shieldOpacity);
